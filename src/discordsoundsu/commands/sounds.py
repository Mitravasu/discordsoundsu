...
class SoundCommands(Cog):
    ...
    @app_commands.command(name="upload", description="Uploads a new sound (Owner only). The filename will be used as the sound key")
    ...
    async def upload(self, interaction: Interaction, attachment: Attachment):
        ...
        if not file_name.endswith('.mp3'):
            return await interaction.response.send_message("Please upload a valid .mp3 file.")

        if attachment.content_type != "audio/mpeg":
            await interaction.response.send_message(f"Skipping {file_name}: not an MP3 audio file.")
    ...
    async def remove(self, interaction: Interaction, sound_name: str):        
        ...
        if file_path.exists():
            ...
            await interaction.response.send_message(f"Could not remove sound {sound_name}")    


    @play.autocomplete('sound_name')
    @remove.autocomplete('sound_name')
    async def sound_name_autocomplete(self, interaction: Interaction, current: str) -> List[app_commands.Choice[str]]:
        """
